...
- 조건문 사용하는 이유
      - 자격증 시험 점수에 따라 ‘합격’, ‘불합격’을 나눠서 보고 싶은 경우
      - 고객 정보 테이블에 성별 데이터가 1, 2로 저장되어 있는데, 데이터를 뽑을 때 각각 ‘여성’, ‘남성’으로 바꾸어 데이터를 확인하고 싶은 경우 등
...
ex1) tips 테이블에서 요일 컬럼을 기준으로 주말과 평일을 나누어 보는 쿼리를 작성해 봅시다.
      ...
      SELECT CASE 
        ...
        END
      FROM tips
ex2) 음식점에 방문한 인원(size)이 5명 이상이면서 주말에 방문한 경우 ‘주말 5인 이상’으로 표시하고, 그렇지 않은 경우 ‘기타’라고 표시하는 쿼리를 작성해 주세요.

      SELECT *,
              CASE            
                  WHEN size >= 5 AND day IN('Sat', 'Sun') THEN "주말 5인 이상"
                  ELSE "기타"
              END
      FROM tips
...
ex1) total_bill 컬럼에 저장된 데이터를 조건에 따라 3단계로 분류해 보려고 합니다. 아래 분류 조건을 참고하여 구간별로 값을 다르게 표시하는 쿼리를 작성해 주세요.
분류 조건 :
- total_bill 컬럼의 값이 40 달러 이상일 때 : ‘큰 금액’
- total_bill 컬럼의 값이 10 달러 이상, 40 달러 미만일 때 : ‘평균 금액’
- total_bill 컬럼의 값이 10 달러 미만일 때 : ‘적은 금액’

      SELECT *,
        CASE 
              WHEN total_bill > 40 THEN '큰 금액'
              WHEN total_bill >= 10 AND total_bill < 40 THEN '평균 금액'
              ELSE '적은 금액' as sales_category
        END
      FROM tips

**조건문(CASE)와 GROUP BY 함께 사용하기**

ex1) tips 테이블에서 요일 컬럼을 기준으로 주말과 평일을 나누어 보는 쿼리를 작성해 봅시다. 그리고 주말과 평일에 방문한 손님들의 수 합계를 계산해 봅시다.
...
ex2) 식사 금액을 3단계로 나누어 데이터를 분류해 봅시다. 각각의 조건에 해당하는 데이터가 총 몇 건 있는지 세어주세요.

      SELECT CASE 
                  WHEN total_bill > 40 THEN '큰 금액'
                  WHEN total_bill >= 10 THEN '평균금액'
                  ELSE '적은 금액'
